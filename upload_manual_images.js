// Script para subir imágenes del manual usando la API del backend
const fs = require('fs');
...
async function uploadManualImages() {
  ...
  // Subir imágenes en lotes de 10 para no sobrecargar el servidor
  const batchSize = 10;
...
if (require.main === module) {
  (async () => {
    ...
    if (!serverRunning) {
      console.error('❌ Error: El servidor no está corriendo en', API_BASE_URL);
      console.log('💡 Asegúrate de iniciar el servidor: npm start (en la carpeta PuraLinoMVC-api)');
      process.exit(1);
    }
    
    console.log('✅ Servidor corriendo, iniciando subida...');
